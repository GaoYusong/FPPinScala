...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(maxTweet: Tweet) : Tweet = {
    right.mostRetweetedAcc(left.mostRetweetedAcc(if (elem.retweets > maxTweet.retweets) elem else maxTweet))
  }
...
class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  def isEmpty = false
   def reverseAcc(acc : TweetList): TweetList = {
    tail.reverseAcc(new Cons(head, acc))
  }
}
...
object GoogleVsApple {
  ...
  def check(patterns : List[String]) : Tweet => Boolean = {
    x : Tweet => {
      patterns.exists(pattern => x.text.contains(pattern))
    }
  }
