...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.map { case (nb, ns) => (nb, ns :: history) }.toStream
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case _ => {
      val newInitial = for {
        (block, moves) <- initial
        newPos <- newNeighborsOnly(neighborsWithHistory(block, moves), explored)
      } yield newPos
      initial ++ from(newInitial, explored ++ newInitial.map { case (b, _) => b }.toSet)
    }
  }
